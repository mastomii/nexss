...
export async function GET(request: NextRequest) {
  ...
  // NOTE: These hooks capture early traffic BEFORE popup is ready. 
  // Once popup injects its hooks (__nxHooked=true), these stop reporting to avoid duplicates.
...
setInterval(function(){
  try{
    ...
    if(curUrl!==lastInjectedUrl){
      ...
      // Send navigation event first, then try to fetch response body separately
      rpt("navigation","GET",curUrl,rawH,null,null,null,null);
      // Try to fetch page content for response body (may fail due to CORS/redirects)
      try{
        fetch(curUrl,{credentials:"include",mode:"same-origin"}).then(function(res){
          ...
          if(ct.indexOf("text")>-1||ct.indexOf("html")>-1||ct.indexOf("json")>-1){
            res.text().then(function(body){
              rpt("navigation","GET",curUrl,rawH,null,resH,body.substring(0,10000),res.status);
            }).catch(function(){});
          }
        }).catch(function(){});
      }catch(e){}
